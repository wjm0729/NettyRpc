...
public class RpcClient implements InitializingBean {
	...
	private ThreadPoolExecutor rpcClientThreadPool;// 负责处理客户端逻辑
	private Map<String, AsyncClientHandler> asyncHandlerMap = Maps.newConcurrentMap();
	private IRpcClientListener rpcClientListener = IRpcClientListener.DEFAULT;
	...
	public void afterPropertiesSet() throws Exception {
		if(rpcClientThreadPool == null) {
			int maxCore = Math.max(4, Runtime.getRuntime().availableProcessors());
			rpcClientThreadPool = new ThreadPoolExecutor(maxCore, maxCore * 2, 600L, TimeUnit.SECONDS, new ArrayBlockingQueue<Runnable>(65536), new NamedThreadFactory("RpcClient"), new RejectedExecutionHandler() {
				@Override
	...
	public AsyncClient sendAsyncMessage(AsyncMessage message) {
	...
	public void submit(Runnable task) {
		if(!rpcClientThreadPool.isShutdown()) {
			rpcClientThreadPool.submit(task);
		}
	}

	public void stop() {
		rpcClientThreadPool.shutdown();
		connectManage.stop();
	}
	...
	public void setRpcClientThreadPool(ThreadPoolExecutor rpcClientThreadPool) {
		this.rpcClientThreadPool = rpcClientThreadPool;
	}
	...
	public IRpcClientListener getRpcClientListener() {
		return rpcClientListener;
	}

	public void setRpcClientListener(IRpcClientListener rpcClientListener) {
